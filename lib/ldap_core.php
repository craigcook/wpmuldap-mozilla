...
class LDAP {
    ...
    function GetLDAPInfo ($type) {
        ...
        if ($type == LDAP_INDEX_EMAIL)          return isset($this->info[0][$mail][0]) ? $this->info[0][$mail][0]: null;
        if ($type == LDAP_INDEX_NAME)           return (isset($this->info[0][$givenname][0]) && isset($this->info[0][$sn][0])) ? $this->info[0][$givenname][0]." ".$this->info[0][$sn][0]: null;
        if ($type == LDAP_INDEX_GIVEN_NAME)     return isset($this->info[0][$givenname][0]) ? $this->info[0][$givenname][0]: null;
        if ($type == LDAP_INDEX_SURNAME)        return isset($this->info[0][$sn][0]) ? $this->info[0][$sn][0]: null;
        if ($type == LDAP_INDEX_PHONE)          return isset($this->info[0][$phone]) ? $this->info[0][$phone]: null;
        if ($type == LDAP_INDEX_HOMEDIR)        return isset($this->info[0][$homedir][0]) ? $this->info[0][$homedir][0]: null;
        if ($type == LDAP_INDEX_MEMBER)         return isset($this->info[0][$member]) ? $this->info[0][$member]: null;
        if ($type == LDAP_INDEX_MACADDRESS)     return isset($this->info[0][$macaddress]) ? $this->info[0][$macaddress]: null;
        // When dealing with "uniqueMember", LDAP actually returns it as "member" - they're synonyms
        if ($type == LDAP_INDEX_UNIQUE_MEMBER)      return isset($this->info[0][$member]) ? $this->info[0][$member]: null;
        if ($type == LDAP_INDEX_DN)             return isset($this->info[0][$dn]) ? $this->info[0][$dn]: null;
